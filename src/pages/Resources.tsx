...
import { Brain, Trophy, Rocket, DollarSign, Shield, Clock, Star, Play, ExternalLink, Users, MessageSquare, Search, CheckCircle2, Circle, ArrowLeft } from 'lucide-react';
    ...
    try {
      ...
      if (error && typeof error === 'object') {
        ...
        if ('hint' in error && error.hint) {
          errorDetails.push(`Hint: ${error.hint}`);
        }
        
        // If no specific properties, try to stringify the error
        if (errorDetails.length === 0) {
          try {
            const errorString = JSON.stringify(error, null, 2);
            if (errorString !== '{}') {
              errorDetails.push(`Error object: ${errorString}`);
            }
          } catch (stringifyError) {
            errorDetails.push(`Error type: ${typeof error}`);
            errorDetails.push(`Error constructor: ${error.constructor?.name || 'Unknown'}`);
          }
        }
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="mb-8">
            <button
              ...
              className="inline-flex items-center gap-2 bg-gradient-to-r from-blue-600 to-purple-600 hover:from-blue-700 hover:to-purple-700 text-white px-6 py-3 rounded-lg font-semibold transition-all duration-300 transform hover:scale-[1.02] shadow-lg shadow-blue-500/25"
            >
          ...
          {!isAuthenticated && (
            <section className="mb-16">
              <div className="bg-gradient-to-r from-blue-500/10 via-purple-500/10 to-pink-500/10 rounded-2xl p-8 backdrop-blur-sm border border-blue-500/20 text-center">
                <h3 className="text-xl font-semibold mb-4">Track Your Learning Progress</h3>
                <p className="text-gray-300 mb-6">
                  Sign in to check off videos as you watch them and track your visualization mastery journey.
                </p>
                <div className="flex justify-center gap-4">
                  ...
                  <button 
                    ...
                    className="border border-blue-500 hover:bg-blue-500/10 text-blue-400 hover:text-blue-300 px-6 py-3 rounded-lg transition-all duration-200"
                  >
                    Create Account
                  </button>
                </div>
